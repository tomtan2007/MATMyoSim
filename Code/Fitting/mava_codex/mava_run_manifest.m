...
if ~isfield(manifest, 'schema_version') || ...
        ...
        ~isfinite(manifest.schema_version)
    error('mava_run_manifest:unsupportedSchemaVersion', ...
        'Run manifest schema version is missing or invalid; version 4 is required.');
end
if manifest.schema_version ~= 4
    error('mava_run_manifest:unsupportedSchemaVersion', ...
        ['Run manifest schema version %g is unsupported; only version 4 ' ...
        'is accepted and no automatic migration is performed.'], ...
...
materialization.results = build_result_materialization(inventory);
materialization.summary = struct('state', 'none', ...
    'active_result_fingerprint', '', ...
...
function records = build_result_materialization(inventory)
records = cell(1, numel(inventory));
for i = 1:numel(inventory)
    item = inventory{i};
    records{i} = struct( ...
...
if isempty(decision.sha256) && ...
        (~isempty(selected_groups) || ~isempty(selected_decisions))
...
elseif ~isempty(decision.sha256)
    ...
    elseif isstruct(groups), groups = num2cell(groups);
    end
    decision_ids = cellfun(@(item) char(string(item.id)), groups, ...
        'UniformOutput', false);
    activate = (islogical(stored_decision.activate) && ...
        isscalar(stored_decision.activate) && stored_decision.activate) || ...
        (isnumeric(stored_decision.activate) && ...
        isscalar(stored_decision.activate) && stored_decision.activate == 1);
    if isempty(selected_groups)
        summary_state = char(string(manifest.materialization.summary.state));
        if activate && ~strcmp(summary_state, ...
                'provisional_optional_requested')
            resume_mismatch(['An unactivated true adaptive decision requires ' ...
                'a provisional summary state.']);
        elseif ~activate && ~isempty(decision_ids)
            resume_mismatch('A false adaptive decision must name no groups.');
        end
    elseif ~activate || ~isequal(decision_ids, selected_groups) || ...
...
if ~isfield(manifest.materialization, 'results') || ...
        numel(manifest.materialization.results) ~= numel(manifest.inventory)
    resume_mismatch('Result materialization does not match inventory size.');
end
results = manifest.materialization.results;
for i = 1:numel(results)
    entry = results{i};
    inventory = manifest.inventory{i};
    validate_result_identity(entry, inventory);
    hash_fields = {'model_best_sha256','best_optimization_sha256', ...
        'fit_results_sha256','status_sha256'};
    populated = cellfun(@(name) ~isempty(entry.(name)), hash_fields);
    files = {fullfile(inventory.result_dir, 'model_best.json'), ...
        fullfile(inventory.result_dir, 'best_optimization.json'), ...
        fullfile(inventory.result_dir, 'fit_results.json'), ...
        fullfile(inventory.result_dir, 'status.json')};
    if ~any(populated)
        completed_status = false;
        if isfile(files{4})
            try
                status = loadjson(files{4});
                completed_status = isfield(status, 'status') && ...
                    strcmp(char(string(status.status)), 'complete');
            catch
            end
        end
        if all(cellfun(@isfile, files(1:3))) || completed_status
            resume_mismatch(sprintf( ...
                'Completed-looking result has no signed materialization: %s.', ...
                inventory.id));
        end
        continue;
    end
    if ~all(populated)
        resume_mismatch(sprintf( ...
            'Completed result has a partial materialization: %s.', ...
            inventory.id));
    end
    for j = 1:numel(files)
        validate_required_hash(files{j}, entry.(hash_fields{j}), ...
...
function validate_result_identity(entry, inventory)
required = {'id','alignment_policy','genotype','stage','restart', ...
    'config_file','result_dir','model_best_sha256', ...
    'best_optimization_sha256','fit_results_sha256','status_sha256'};
if ~isstruct(entry) || ~all(isfield(entry, required)) || ...
        ~strcmp(char(string(entry.id)), char(string(inventory.id))) || ...
        ~strcmp(char(string(entry.alignment_policy)), ...
        char(string(inventory.alignment_policy))) || ...
        ~strcmp(char(string(entry.genotype)), char(string(inventory.genotype))) || ...
        ~strcmp(char(string(entry.stage)), char(string(inventory.stage))) || ...
        ~isscalar(entry.restart) || entry.restart ~= inventory.restart || ...
        ~strcmp(char(string(entry.config_file)), ...
        char(string(inventory.config_file))) || ...
        ~strcmp(char(string(entry.result_dir)), ...
        char(string(inventory.result_dir)))
    resume_mismatch(sprintf( ...
        'Result materialization identity mismatch: %s.', inventory.id));
end
end
