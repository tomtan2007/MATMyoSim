...
assert(isfinite(L.baseline_value));
baseline_range = L.trough_index:max(L.trough_index, L.rise_index-1);
assert(~isempty(baseline_range));
assert(abs(L.baseline_value - mean(y(L.trough_index:L.rise_index-1))) < 1e-12);
