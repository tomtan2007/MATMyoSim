function parameter_sweep_all_4state_control
% Sweeps ALL kinetic parameters of the best-fit 4-state control model.
...
% Saves to temp/sweeps_all/sweep_all_4state_control.png
...
% Filter to only params that exist in this model
valid = {};
for i = 1:numel(sweep_params)
    if isfield(params_best, sweep_params{i})
        valid{end+1} = sweep_params{i}; %#ok<AGROW>
    else
...
for pi = 1:n_params
    ...
    for mi = 1:n_mult
        ...
        tmp_model = fullfile(fileparts(mfilename('fullpath')), 'temp', sprintf('sweep_4sc_%s_m%d.json', pname, mi));
...
sgtitle('4-state Control: all-parameter sweep (red * = best-fit)', 'FontSize', 11);

out_file = fullfile(OUT_DIR, 'sweep_all_4state_control.png');
...
savejson('', results, fullfile(OUT_DIR, 'sweep_all_4state_control_results.json'));
fprintf('Done — 4-state control sweep complete.\n');
end
