function parameter_sweep_all_6state_HCM
% Sweeps ALL kinetic parameters of the best-fit 6-state HCM model.
% Each parameter is varied at x0.1, x0.316, x1.0, x3.16, x10 of best-fit.
% Zero-valued params (k_8, k_10) are swept from a small baseline (0.01).
% Produces a 2-row figure: top=peak force, bottom=pre-activation SRX (M1+M6).
% Saves to temp/sweeps_all/sweep_all_6state.png
...
% For zero-valued params, use this as the sweep baseline
ZERO_BASELINE = 0.01;
...
for pi = 1:n_params
    ...
    for mi = 1:n_mult
        ...
        tmp_model = fullfile(fileparts(mfilename('fullpath')), 'temp', sprintf('sweep_6s_%s_m%d.json', pname, mi));
...
for pi = 1:n_params
    ...
    hold on;
    % For zero-params, best-fit point is at x1 (mult=1.0 of baseline, not of zero)
...
sgtitle('6-state HCM: all-parameter sweep (red * = best-fit; x-axis = multiplier of best-fit)', 'FontSize', 11);

out_file = fullfile(OUT_DIR, 'sweep_all_6state.png');
...
savejson('', results, fullfile(OUT_DIR, 'sweep_all_6state_results.json'));
fprintf('Done — 6-state sweep complete.\n');
end
