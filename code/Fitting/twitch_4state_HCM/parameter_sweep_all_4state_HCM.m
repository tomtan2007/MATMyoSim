function parameter_sweep_all_4state_HCM
% Sweeps ALL kinetic parameters of the best-fit 4-state HCM model.
...
% Saves to temp/sweeps_all/sweep_all_4state.png
...
% All kinetic parameters; zero-valued ones use baseline 0.01 for sweep
sweep_params = {'k_1', 'k_force', 'k_2', 'k_3', 'k_4_0', 'k_4_1', ...
                'k_5_0', 'k_5_1', 'k_6_0', 'k_6_1', 'k_7_0', 'k_7_1', ...
                'k_8', 'k_cb', 'k_on', 'k_off', 'k_coop'};

% Remove params that don't exist in this model's template
valid = {};
for i = 1:numel(sweep_params)
    pn = sweep_params{i};
    if isfield(params_best, pn)
        valid{end+1} = pn; %#ok<AGROW>
    else
        fprintf('Skipping %s (not in model)\n', pn);
    end
end
sweep_params = valid;
...
for pi = 1:n_params
    ...
    for mi = 1:n_mult
        ...
        tmp_model = fullfile(fileparts(mfilename('fullpath')), 'temp', sprintf('sweep_4s_%s_m%d.json', pname, mi));
...
fig = figure('Visible', 'off', 'Position', [50 50 min(1800, 120*n_params) 800]);
...
sgtitle('4-state HCM: all-parameter sweep (red * = best-fit)', 'FontSize', 11);

out_file = fullfile(OUT_DIR, 'sweep_all_4state.png');
...
savejson('', results, fullfile(OUT_DIR, 'sweep_all_4state_results.json'));
fprintf('Done — 4-state sweep complete.\n');
end
