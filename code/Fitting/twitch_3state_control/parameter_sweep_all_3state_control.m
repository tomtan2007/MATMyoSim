function parameter_sweep_all_3state_control
% Sweeps ALL kinetic parameters of the best-fit 3-state control model.
...
% Saves to temp/sweeps_all/sweep_all_3state_control.png
...
for pi = 1:n_params
    ...
    for mi = 1:n_mult
        ...
        tmp_model = fullfile(fileparts(mfilename('fullpath')), 'temp', sprintf('sweep_3sc_%s_m%d.json', pname, mi));
...
sgtitle('3-state Control: all-parameter sweep (red * = best-fit)', 'FontSize', 11);

out_file = fullfile(OUT_DIR, 'sweep_all_3state_control.png');
...
savejson('', results, fullfile(OUT_DIR, 'sweep_all_3state_control_results.json'));
fprintf('Done — 3-state control sweep complete.\n');
end
