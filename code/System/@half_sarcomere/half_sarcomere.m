classdef half_sarcomere < handle
    ...
    methods
        ...
        function obj = half_sarcomere(varargin)
            ...
            % Re-initialize M1/M2 at SRX/DRX equilibrium (k_2/(k_1+k_2))
            % so the pre-Ca period starts flat rather than transiting from M1=1.
            % Only applies to kinetic schemes that have an explicit SRX state.
            if isfield(obj.parameters, 'k_1') && isfield(obj.parameters, 'k_2') && ...
               (startsWith(obj.kinetic_scheme, '3state_with_SRX') || ...
                startsWith(obj.kinetic_scheme, '4state_with_SRX') || ...
                startsWith(obj.kinetic_scheme, '6state_with_SRX_and_titin'))
                k1_val = obj.parameters.k_1;
                k2_val = obj.parameters.k_2;
                srx_eq = k2_val / (k1_val + k2_val);
                drx_eq = k1_val / (k1_val + k2_val);
                obj.myofilaments.y(1) = srx_eq;
                obj.myofilaments.y(2) = drx_eq;
            end
